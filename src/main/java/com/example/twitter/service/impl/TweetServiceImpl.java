...
public class TweetServiceImpl implements TweetService {
    ...
    public Tweet createTweet(Tweet tweet) {
        if (tweet.getText().length() == 0 || tweet.getText().length() > 280) {
            throw new ApiRequestException("Incorrect tweet text length", BAD_REQUEST);
        }
    ...
    public void parseHashtagInText(Tweet tweet) {
    ...
    public TweetProjection createPoll(Long pollDateTime, List<String> choices, Tweet tweet) {
        if(choices.size() < 2){
            throw new ApiRequestException("Incorrect poll choices", BAD_REQUEST);
        }
        Tweet createdTweet = createTweet(tweet);
        ...
        choices.forEach(choice -> {
            if(choice.isEmpty() || choice.length() > 25){
                throw new ApiRequestException("Incorrect choice text length",BAD_REQUEST);
            }
    ...
    public TweetProjection updateScheduledTweet(Tweet tweetInfo) {
        if(tweetInfo.getText().isEmpty() || tweetInfo.getText().length() > 280){
            throw new ApiRequestException("Incorrect text length", BAD_REQUEST);
        }
        Tweet tweet = tweetRepository.findById(tweetInfo.getId()).orElseThrow(
    ...
    public Tweet deleteTweet(Long tweetId) {
        User user = authenticationService.getAuthenticatedUser();
        Tweet tweet = user.getTweets().stream().filter(t -> t.getId().equals(tweetId)).findFirst().get();
        ...
        tweet.getReplies().forEach(reply -> reply.getUser().getTweets().removeIf(replyingTweet -> replyingTweet.equals(reply)));
        ...
        List<Notification> notifications = user.getNotifications().stream()
                ...
                && notification.getTweet().getId().equals(tweet.getId())).toList();
        ...
        if(tweet.getAddressedTweetId() != null){
            ...
            Tweet reply = addressedTweetReplies.stream().filter(r -> r.equals(tweet)).findFirst().get();
    ...
    public List<TweetProjection> searchTweets(String text) {
        return tweetRepository.findAllByText(text).stream().map(TweetsProjection::getTweet).toList();
    }
    ...
    public Map<String, Object> retweet(Long tweetId) {
        ...
        Tweet tweet = tweetRepository.findById(tweetId).orElseThrow(
                () -> new ApiRequestException("Tweet not found", BAD_REQUEST)
        );
        List<Retweet> retweets = user.getRetweets();
        Optional<Retweet> retweet = retweets.stream().filter(t -> t.getTweet().getId().equals(tweetId)).findFirst();
